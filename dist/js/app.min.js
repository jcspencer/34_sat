...
jQuery(document).ready(function(s) {
    var i = 1170;
    if (s(window).width() > i) {
        var a = s(".navbar-custom").height();
        s(window).on("scroll", {
            previousTop: 0
        }, function() {
            var i = s(window).scrollTop();
            i < this.previousTop ? i > 0 && s(".navbar-custom").hasClass("is-fixed") ? s(".navbar-custom").addClass("is-visible") : s(".navbar-custom").removeClass("is-visible is-fixed") : i > this.previousTop && (s(".navbar-custom").removeClass("is-visible"), i > a && !s(".navbar-custom").hasClass("is-fixed") && s(".navbar-custom").addClass("is-fixed")), this.previousTop = i
        })
    }
});
var margin = {
...
d3.csv("bar-data.csv", function(t, a) {
    a.forEach(function(t) {
        t.date = parseDate(t.date), t.value = +t.value
    }), x.domain(a.map(function(t) {
        return t.date
    })), y.domain([0, d3.max(a, function(t) {
        return t.value
        ...
        return x(t.date)
    }).attr("width", x.rangeBand()).attr("y", function(t) {
        return y(t.value)
    }).attr("height", function(t) {
        return height - y(t.value)
    })
});
var url = "https://jcspencer.github.io/34_sat/dist/data/out.geo.json",
    opts = {
        center: [-37, 144],
        zoom: 6,
        dragging: !1,
    ...
    map = new L.Map("map", opts).addLayer(new L.TileLayer("http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png")),
    svg = d3.select(map.getPanes().overlayPane).append("svg"),
    g = svg.append("g").attr("class", "leaflet-zoom-hide"),
    tip = d3.tip().attr("class", "d3-tip").offset([-10, 0]).html(function(t) {
        return "<strong>Average: " + t.properties.NAME + " </strong></span>"
    });
svg.call(tip), d3.json(url, function(t, e) {
    function a(t) {
    ...
    function o() {
        var t = s.bounds(e),
            a = t[0],
            o = t[1];
        svg.attr("width", o[0] - a[0]).attr("height", o[1] - a[1]).style("left", a[0] + "px").style("top", a[1] + "px"), g.attr("transform", "translate(" + -a[0] + "," + -a[1] + ")"), p.attr("d", s)
    }
    ...
    if (t) throw t;
    var n = d3.geo.transform({
            point: r
        }),
        s = d3.geo.path().projection(n),
        p = g.selectAll("path").data(e.features).enter().append("path").attr("class", "n").attr("class", function(t) {
            return "region_" + t.properties.NAME
        }).on("mouseover", tip.show).on("mouseout", tip.hide).on("click", a);
    map.on("viewreset", o), o()
}); +
